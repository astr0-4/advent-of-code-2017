...
"""
    ...
    func convertInputInto2DArray(input: String) -> [[Int]] {
        let splitInput = input.split(separator: "\n")
        return splitInput.map { $0.components(separatedBy: "    ").flatMap { Int($0) } }
    }

    func getChecksums(inputArray: [[Int]]) -> [Int] {
        let inputArray = convertInputInto2DArray(input: input)
        return inputArray.map { item -> Int in
            let sortedItems = item.sorted()
            if let smallest = sortedItems.first, let biggest = sortedItems.last {
    ...
    public func getPart1Solution() -> Int {
        let inputArray = convertInputInto2DArray(input: input)
        return sumNums(getChecksums(inputArray: inputArray))
    }

    // MARK: - Part 2
    func getEvenlyDivisibleValue(arr: [Int]) -> Int {
    ...
    func getPart2Checksum(arr: [[Int]]) -> Int {
        let rowValues = arr.map{ getEvenlyDivisibleValue(arr: $0) }
        return sumNums(rowValues)
    }


    public func getPart2Solution() -> Int {
        let inputArray = convertInputInto2DArray(input: input)
        return getPart2Checksum(arr: inputArray)
    }
